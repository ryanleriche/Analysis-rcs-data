...
ft_data_file = fullfile(save_dir, [pt_side_id, '_ft_form_TD_struct.mat']);
...
h.Marker = '.';
title(sprintf('%s | longest continuous TD from each streaming session', pt_side_id));


epoching_txt = sprintf(['min(session duration), %g min             \n',...
                        'epoch, [%g, %g] min (survey at 0)          \n',...
                         'min(epoch duration), %.2g min              \n',...
                     ], ...
                     minutes([cfg.min_sess_dur, -cfg.r_cap_cent_toi, cfg.epoch_dur])...
                        );
