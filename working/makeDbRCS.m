function [RCSdatabase_out, varargout] = makeDbRCS(dirname, ptIDside, varargin)
%{
function database_out = makeDbRCS(dirname)
...
%%
for d = 1 : length(scbs_sess_dirs )

    % run through all of sessions (aDBS then SCBS sessions)
    given_sess = findFilesBVQX(scbs_sess_dirs {d},'Device*',struct('dirs',1,'depth',1));


    fprintf('Reading folder %d of %d  \n', d, length(scbs_sess_dirs))

    if isempty(given_sess) % no data exists inside

        db_out(d).time = [];
        db_out(d).matExist  = 0;
        [~,fn] = fileparts(scbs_sess_dirs{d});
        ...
        td_file      = findFilesBVQX(scbs_sess_dirs{d},'EventLog.json');
        dev_file     = findFilesBVQX(scbs_sess_dirs{d},'DeviceSettings.json');
        ...
        if ~isempty(td_file) && ~isempty(dev_file)  % time data file doesn't exist
            ...
            settingsfile                 = findFilesBVQX(scbs_sess_dirs{d},'DeviceSettings.json');
            ...
            try
                ...
                % IF there is time/ power domain data
                    if ~isempty(timeDomainSettings) && ~isempty(powerSettings)
                        ...
                        db_out(d).duration  = duration(timeStart-timeStop,'Format','hh:mm:ss.SSS');
        ...
        try
            ...
            if db_out(d).stimSettingsOut.therapyStatus == 1 
                ...
                stimfile          = findFilesBVQX(scbs_sess_dirs{d},'StimLog.json');
        ...
        try
            prt_folder                 = findFilesBVQX(scbs_sess_dirs{d},'EventLog.json');
...
RCSdatabase_out         = sortrows(database_out, 'sessname'); %sorting by session name
...
if iscell(RCSdatabase_out.timeStart)
    ...
    loc= isempty(RCSdatabase_out.time);
end
