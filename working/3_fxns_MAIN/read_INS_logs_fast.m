...
added reporting of state definiation by Ryan Leriche
July 14 2023
%}
...
if ~isempty(str)  % only continue if the file is not empty
  ...
  %%
    if endsWith(fn,"EventLog.txt")
       ...
       % remove events that repeat (include first and last entry)
...
elseif endsWith(fn, "AppLog.txt")
    ...
    %%% AdaptiveTherapyStateWritten

    i_change  = strcmp(events_tbl.event_id,'AdaptiveTherapyStateWritten');
    events    = events_raw(i_change);
    
    if ~isempty(events)

        state_def_tbl = table;
        state_def_tbl.time =  get_date_from_hexstring(events); % see subfunction below
         
    
        % get state 
        xpr     = 'AdaptiveTherapyStateWrittenEntry.State ';
        cac1    = cellfun(@(x) regexp(x, xpr), events,'UniformOutput',false);
    
    
        state_def_tbl.state = cellfun(@(x, y) x(y+58:y+63), events, cac1, 'UniformOutput',false);
    
    
        for h = 0:3
            xpr     = sprintf('AdaptiveTherapyStateWrittenEntry.Prog%gAmpTarget',h);
            cac1    = cellfun(@(x) regexp(x, xpr), events,'UniformOutput',false);
            
            tmp_mA = cellfun(@(x, y) hex2dec(strtrim(x(y+51:y+65))), events, cac1, 'UniformOutput',false);
            
            % LSB for mA is 0.1
            state_def_tbl.(sprintf('prog%gmA',h)) = [tmp_mA{:}]'/10;
    
        end
    
          xpr     = 'AdaptiveTherapyStateWrittenEntry.RatePeriodTarget';
          cac1    = cellfun(@(x) regexp(x, xpr), events,'UniformOutput',false); 
          
          tmp_period =  cellfun(@(x, y) hex2dec(strtrim(x(y+51:y+66))), events, cac1, 'UniformOutput',false);
    
          % LSB for period is 10 µs
          state_def_tbl.rateHz = (1 ./[tmp_period{:}]') * 10^5;
    else
        state_def_tbl = {'empty'};
    end
