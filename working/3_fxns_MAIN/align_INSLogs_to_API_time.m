...
% for processed hemisphere's EventLog.txt, group changes and therapy status
proc_g_changes...
    ...
    = find_nearest_ss_to_INS_logs_2(...
    ...
...
% for processed hemisphere's AppLog.txt, aDBS states
proc_app...
    ...
    = find_nearest_ss_to_INS_logs_2(...
    ...
...
% for processed hemisphere's EventLog.txt, recharge sessions
INS_logs_out.recharge...
    ...
    = find_nearest_ss_to_INS_logs_2(...
    ...
par_db_RCSXXX, INS_logs_RCSXXX.recharge);

% for processed hemisphere's AppLog.txt, LD detections
INS_logs_out.ld_detect...
    ...
    = find_nearest_ss_to_INS_logs_2(...
    ...
par_db_RCSXXX, INS_logs_RCSXXX.ld_detect);
...
for i_app = 1 : height(proc_app)
    ...
    if ~isempty(i_grp_chan)  % group changes the occured BETWEEN AppLog.txt entries
        ...
        for j = 1 :  length(i_grp_chan)
            ...
            if strcmp(tmp_grp.prev_sess_name, 'No Entry'); continue; end
        ...
        if ~isempty(inter_grp_tbl)
            inter_grp_tbl(isnat(inter_grp_tbl.time_INS),:) = [];
...
to_add_proc_app(cellfun(@isempty, to_add_proc_app)) = [];
