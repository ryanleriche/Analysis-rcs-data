...
for i = 1   :   length(cfg.pt_sides)
    ...
    db_RCSXXX             = db.(cfg.pt_sides{i});
    
    %% option to add new streaming sessions rather than building from scratch
   
    outputFileName    = fullfile(cfg.proc_dir,[cfg.pt_sides{i} '_parsed_database.mat']);
    ...
    if isfile(outputFileName) && ~cfg.ignoreold
        ...
        if isempty(db_RCSXXX)
            ...
            fprintf('%s | No new data since %s!\n        --> Existing database returned\n', cfg.pt_sides{i}, old.par_db_RCSXXX.timeStart(end));
    
            par_db.(cfg.pt_sides{i})     = old.par_db_RCSXXX;
            
            % from sess_name -> duration onwards
            ss_var_oi.(cfg.pt_sides{i})  = old.par_db_RCSXXX.Properties.VariableNames(5:end);
            continue
    
        else
            fprintf('%s | new RCS folders from %s --> %s\n', cfg.pt_sides{i}, db_RCSXXX.timeStart{1}, db_RCSXXX.timeStart{end});
    ...
    for d=1:4
        LD0_feat = struct2table(cellfun(@(x) x(d), LD0_tbl.features));
    ...
    for d=1:4
        LD1_feat = struct2table(cellfun(@(x) x(d), LD1_tbl.features));
    ...
    %%
    writetable(par_db_RCSXXX,fullfile(cfg.proc_dir,[cfg.pt_sides{i}...
        '_parsed_database.xlsx']));
    
    save(fullfile(cfg.proc_dir,[cfg.pt_sides{i} '_parsed_database.mat']),...
        'par_db_RCSXXX');
    
    fprintf('%s | parsed database saved to %s%s_parsed_database.mat\n',...
        cfg.pt_sides{i}, cfg.proc_dir, cfg.pt_sides{i});

    par_db.(cfg.pt_sides{i})      = par_db_RCSXXX;
    ss_var_oi.(cfg.pt_sides{i})   = exp_sense_state_vars;
end
