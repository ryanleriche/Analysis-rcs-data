function s0_REDcap    = append_pain_maps(s0_map_dir, s0_REDcap, visits_tbl)
...
for i = 1 : length(pt_ids)
    ...
    % define pain body map (PBM) metrics
    redcap.PBM_sumpixelval    = nan(height(redcap), 1);
    redcap.PBM_pixelspread    = nan(height(redcap), 1);
    redcap.PBM_avgpixelval    = nan(height(redcap), 1);
    ...
    for j = 1 : height(tmp_tbl)
        ...
        redcap.PBM_sumpixelval(i_map)       = tmp_tbl.pixelvalue(j);
        redcap.PBM_pixelspread(i_map)       = tmp_tbl.painspread(j);
        redcap.PBM_avgpixelval(i_map)       = tmp_tbl.avgpainintensity(j);
    
    end

    if length(find(~isnan(redcap.PBM_sumpixelval))) == ...
    ...
    redcap.PBM_pixelspread       = redcap.PBM_pixelspread * 100;
    ...
    if any(strcmp(pt_ids{i}, {'RCS02', 'RCS04', 'RCS07'}))
        ...
        redcap.PBM_sumpixelval   = redcap.PBM_sumpixelval / (820452);

    elseif any(strcmp(pt_ids{i}, {'RCS05', 'RCS06'}))

        redcap.PBM_sumpixelval   = redcap.PBM_sumpixelval / (724608);
