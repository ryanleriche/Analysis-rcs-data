function [db, bs] = makeDatabaseRCS_Ryan(cfg)
%{
...
for i = 1:length(cfg.pt_sides)
    ...
    pt_rootdir  = fullfile(cfg.raw_dir,char(regexp(cfg.pt_sides{i},...
                            '\w*\d\d','match'))); 
    
    
    %  Define the directories to search in (SCBS and aDBS)
    scbs_dir_root       = fullfile(pt_rootdir,'/SummitData/SummitContinuousBilateralStreaming/', cfg.pt_sides{i});
    adbs_dir_root       = fullfile(pt_rootdir, '/SummitData/StarrLab/', cfg.pt_sides{i});
    ...
    outputFileName    = fullfile(cfg.proc_dir,[cfg.pt_sides{i} '_database.mat']);
    
    
    if isfile(outputFileName) && ~cfg.ignoreold
    
        fprintf('%s | Loading previously saved database', cfg.pt_sides{i});
        ...
        if isempty(sess_dirs)
            ...
            db.(cfg.pt_sides{i})  =  old_database;
            bs.(cfg.pt_sides{i})  =  old_badsessions;
    ...
    %d = find(contains(sess_dirs, 'Session1677519987408'))
    ...
    for k=1:length(var_class)
        ...
        if any(...
            ...
            RCSdatabase_out = removevars(RCSdatabase_out , var_names{k});
            RCSdatabase_out = renamevars(RCSdatabase_out ,[var_names{k},'_new'], var_names{k});
    ...
    % writetable(RCSdatabase_out,fullfile(cfg.proc_dir,[cfg.pt_sides{i}...
    %     '_database.csv']));
    
    save(fullfile(cfg.proc_dir,[cfg.pt_sides{i} '_database.mat']),...
        'RCSdatabase_out','badsessions');
    
    fprintf('mat of database saved as %s to %s \n',...
        [cfg.pt_sides{i} '_database.mat'],cfg.proc_dir);
    
    %==================================================================
    db.(cfg.pt_sides{i})  =  RCSdatabase_out;
    bs.(cfg.pt_sides{i})  =  badsessions;
end
end