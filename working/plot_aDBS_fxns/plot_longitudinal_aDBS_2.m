function  long_aDBS_tbl = plot_longitudinal_aDBS_2(cfg, pt_side_id, REDcap, INS_logs, app_SS_tbl)

% cfg             = [];
% cfg.dates       = 'AllTime';
% cfg.anal_dir    = [pia_dir, 'processed/aDBS_offline_sessions/'];
% 
% cfg.dates          = 'DateRange';
...
% see cfg.anal_dir for the aDBS longitudinal plots)
close all
...
save_dir        = [cfg.anal_dir,'/aDBS_offline_sessions/', pt_side_id ,'/'];

if ~isfolder(save_dir);     mkdir(save_dir);           end

%%% return all previously made file/folder names
tmp_dirs         = struct2table(dir(save_dir));
plted_aDBS_files = tmp_dirs(~ismember(tmp_dirs.name,{'.','..','.DS_Store'}),:);

plted_aDBS_files.path            = cellfun(@(x,y) fullfile(x,y), ...
                                            plted_aDBS_files.folder, plted_aDBS_files.name,...
                                            'UniformOutput',false);
plted_aDBS_files.prev_date_range = cellfun(@(x) x(1:19), plted_aDBS_files.name, 'UniformOutput',false);
...
long_aDBS_tbl  = table;
u_settings     = unique(ss_tbl_oi.sess_setting_changes);
...
for i = 1  :  length(u_settings)
    ...
    %--> interpolate aDBS state/stim for finegrained duty cycle reporting
    step_dur        = duration('00:00:10');
    t_vec           = start_time:step_dur:stop_time;

    % initalize state, amplitude, rate (stim frequency), and nearest,
    % prececing, streaming session vectors
    ...
    for h = 0:  n_sub_sess
        ...
        subplot(5,3, 12:15)
                ...
                yyaxis right
                    ...
                    stairs(plt_app_oi.time_INS(1:end-1), plt_app_oi.oldstate(2:end), ...
                        '-','LineWidth',1.25, 'Color',  color_mA);

                    ylim([-.5,8.5]); yticks(0:8); ylabel('State');    grid on;
        ...
        switch cfg.plt_state_dur
            ...
            case 'two_chunks'
                ...
                set(gca, 'TickLength', [0, 0],'FontSize', 12, 'YColor', color_mA, ...
       ...
       %%% make folders based on sub-session times, and save as .pngs
        ...
        % defined at start from cfg.anal_dir as specified in wrapper
        i_date_end = regexp(offline_sess_name, '-->')-1;

        end_file_date      = datetime(offline_sess_name(1:i_date_end), 'InputFormat', 'yy-MM-dd');

        all_end_file_dates = cellfun(@(x) datetime(x(i_date_end+4:end), 'InputFormat', 'yy-MM-dd'),...
                              plted_aDBS_files.prev_date_range);
     
        % delete is exact file already exists
        if isfile([save_dir,filename]); delete([save_dir,filename]); end

        % delete if any of the previous files have end date BEFORE this
        % files end date
        if ~isempty(all_end_file_dates)
            i_old = find(le(end_file_date, all_end_file_dates));
            if ~isempty(i_old) && h ==0
                for q = 1 : length(i_old)
    
                    to_delete = plted_aDBS_files.path{i_old(q)};
                    if isfolder(to_delete);   rmdir(to_delete,'s')
                    else;                     delete(to_delete);end
                end
            end
        end
