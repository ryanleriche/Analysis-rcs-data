...
plt_pwrspectra_by_sess = nan(4, length(t_vec), length(fft_bins_inHz));
...
for i_day = 1 :length(t_vec)-1
    ...
    plt_pwrspectra_by_sess(:, i_day, :) ...
        =...
        mean(pwrspectra_by_sess(:, i_ss, :), 2);
    ...
    for i_ch = 1:4
        ...
        if ~cfg.z_score
            if j_plt == 1
                ...
                [lineout, ~] = stdshade(plt_spectrum, 0.2, 'k', fft_bins_inHz(i_freq_oi));
